...
class AccountService {
  ...
  async getAccountInfo() {
    try {
      ...
      return {
        initialBalance: this.userAccount.initialBalance,
        currentBalance: this.userAccount.currentBalance,
        totalAssets: this.userAccount.currentBalance,
        totalProfit: 0,
        profitPercentage: 0
      };
    }
   ...
   * 买入股票
   */
  async buyStock(ticker, quantity, price = null) {
    try {
      const isValid = await this.stockDataService.isValidTicker(ticker);
      if (!isValid) {
      ...
      const stockData = await this.stockDataService.getLatestStockData(ticker);
      if (!stockData) {
      ...
      if (existingHolding) {
        ...
        await this.dbService.updateHolding(ticker, {
          shares: newQuantity,
          cost_price: newCostPrice,
          current_price: currentPrice,
          profit_loss_amount: newProfitLoss,
          profit_loss_percent: newProfitLossPercent,
          investment_status: 'HOLDING'
        });
      } else {
        // 创建新持仓
        ...
        await this.dbService.insertHolding({
          stock_code: ticker,
   ...
   * 获取可买入的最大数量
   */
  async getMaxBuyQuantity(ticker) {
    try {
      const stockData = await this.stockDataService.getLatestStockData(ticker);
      if (!stockData) {
        return 0;
