...
class AccountService {
  constructor() {
    ...
    // 初始化一些测试持仓
    this.initializeTestHoldings();
  }

  /**
   * 初始化测试持仓
   */
  initializeTestHoldings() {
    // 添加一些测试持仓
    this.buyStock('GLE FP', 100, 23.5);
    this.buyStock('KPN NA', 200, 3.2);
    this.buyStock('AALB NA', 50, 37.0);
    
    logger.info('初始化测试持仓完成');
  }
  ...
  calculateTotalAssets() {
    ...
    for (const [ticker, holding] of this.userAccount.holdings) {
      const stockData = this.stockDataService.getLatestStockData(ticker);
      if (stockData) {
        stockValue += holding.quantity * stockData.currentPrice;
      }
    }
  ...
  getPortfolio() {
    ...
    for (const [ticker, holding] of this.userAccount.holdings) {
      ...
      if (stockData) {
        ...
        const profitPercentage = (profit / holding.totalCost) * 100;
